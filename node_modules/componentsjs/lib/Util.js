...
const RdfStreamParser_1 = require("./rdf/RdfStreamParser");
...
const RdfStreamIncluder_1 = require("./rdf/RdfStreamIncluder");
...
class Util {
     ...
     * @private
     */
    static getContentsFromUrlOrPath(path, fromPath) {
        return new Promise((resolve, reject) => {
            let parsedUrl = url.parse(path);
            let separatorPos = path.indexOf(':');
            if ((separatorPos >= 0 && separatorPos < path.length && path.charAt(separatorPos + 1) === '\\')
                ...
                try {
                    var request = (parsedUrl.protocol == 'https:' ? https : http).request(parsedUrl, (data) => {
     ...
     * @private
     */
    static parseRdf(rdfDataStream, path, fromPath, ignoreImports, absolutizeRelativePaths, contexts, importPaths) {
        if (!fromPath)
            fromPath = Path.dirname(path);
        let stream = new RdfStreamParser_1.RdfStreamParser(contexts).pipeFrom(rdfDataStream);
        let ret = stream.pipe(new RdfStreamIncluder_1.RdfStreamIncluder(Util, fromPath, !ignoreImports, absolutizeRelativePaths, contexts, importPaths));
        stream.on('error', (e) => ret.emit('error', Util.addFilePathToError(e, path || fromPath, path ? fromPath : null)));
        return ret;
    }
    /**
     ...
     */
    static captureType(value, param) {
        if (!value)
            return value;
        let raw = value.value;
     ...
     */
    static setMainModulePath(path) {
        Util.MAIN_MODULE_PATH = fs.realpathSync(path);
        const sections = Util.MAIN_MODULE_PATH.split(Path.sep);
        const paths = [];
    ...
    static initDefaultMainModulePath() {
        for (let nodeModulesPath of global.process.mainModule.paths) {
     ...
     */
    static setMainModulePaths(paths) {
     ...
     */
    static getAvailableNodeModules(path, cb, ignorePaths) {
        if (Util.cachedAvailableNodeModules[path]) {
            ...
            Util.cachedAvailableNodeModules[path] = new Promise((resolve, reject) => {
                const paths = [];
                if (!ignorePaths)
                    ignorePaths = {};
                    ...
                    if ((yield stat(Path.join(path, 'package.json'))).isFile()) {
                        ...
                        for (const module of modules) {
                            ...
                            if (!module.startsWith('.')) {
                                ...
                                if (module.startsWith('@')) {
                                    const scopedModules = yield readdir(modulePath);
                                    for (const scopedModule of scopedModules) {
                                        yield recurse(Path.join(modulePath, scopedModule), cb);
                                    }
                                }
                                else {
                                    yield recurse(modulePath, cb);
                                }
                            }
                        }
                    }
     ...
     */
    static getPackageJson(path) {
        let data = Util.NODE_MODULES_PACKAGE_CONTENTS[path];
        if (!data) {
     ...
     */
    static getModuleComponentPaths(path) {
        return new Promise((resolve, reject) => {
            let data = {};
            Util.getAvailableNodeModules(path, (modulePath) => {
                ...
                let pckg = Util.getPackageJson(Path.join(modulePath, 'package.json'));
                if (pckg) {
                    let currentModuleUri = pckg['lsd:module'];
                    let relativePath = pckg['lsd:components'];
    ...
    static getContextPaths(path) {
        return new Promise((resolve, reject) => {
            ...
            Util.getAvailableNodeModules(path, (modulePath) => {
                ...
                let pckg = Util.getPackageJson(Path.join(modulePath, 'package.json'));
                if (pckg) {
                    let contexts = pckg['lsd:contexts'];
                    if (contexts) {
                        _.forOwn(contexts, (value, key) => {
                            if (!(key in data)) {
                                let filePath = Path.join(modulePath, value);
                                data[key] = fs.readFileSync(filePath, 'utf8');
                                // Crash when context is invalid JSON
                                try {
                                    let context = JSON.parse(data[key]);
                                    jsonld.compact({}, context, (e) => {
                                        if (e) {
     ...
     * @return A promise resolving to a mapping of context URL to context contents
     */
    static getAvailableContexts(scanGlobal) {
        return new Promise((resolve, reject) => {
            let globalPath = scanGlobal ? globalModules : null;
            let paths = Util.getMainModulePaths();
            if (paths) {
    ...
    static getImportPaths(path) {
        return new Promise((resolve, reject) => {
            ...
            Util.getAvailableNodeModules(path, (modulePath) => {
                ...
                let pckg = Util.getPackageJson(Path.join(modulePath, 'package.json'));
                if (pckg) {
                    let contexts = pckg['lsd:importPaths'];
                    if (contexts) {
                        _.forOwn(contexts, (value, key) => {
                            if (!(key in data)) {
     ...
     */
    static addFilePathToError(e, filePath, fromPath) {
...
Util.MAIN_MODULE_PATH = null;
Util.MAIN_MODULE_PATHS = null;
Util.cachedAvailableNodeModules = {};
module.exports = Util;
