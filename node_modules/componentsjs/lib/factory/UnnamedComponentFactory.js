...
class UnnamedComponentFactory {
    constructor(componentDefinition, constructable, overrideRequireNames, componentRunner) {
        ...
        this._componentRunner = componentRunner || new Loader_1.Loader();
        // Validate params
    ...
    _validateParam(resource, field, type, optional) {
        if (!resource[field]) {
            if (!optional) {
    ...
    static getArgumentValue(value, componentRunner, settings) {
        settings = settings || {};
        return new Promise((resolve, reject) => {
            if (value.fields) {
                // The parameter is an object
                return Promise.all(value.fields.map((entry) => {
                    if (!entry.k) {
                ...
                return Promise.all(value.elements.map((entry) => {
                    if (!entry.v) {
                ...
                return Promise.all(value.map((element) => UnnamedComponentFactory.getArgumentValue(element, componentRunner, settings)))
                    .then(resolve).catch(reject);
            }
    ...
    makeArguments(settings) {
        return this._componentDefinition.arguments ? Promise.all(this._componentDefinition.arguments.list
            .map((resource) => resource ? UnnamedComponentFactory.getArgumentValue(resource, this._componentRunner, settings) : undefined)) : Promise.resolve([]);
    }
    /**
     ...
     * @private
     */
    _requireCurrentRunningModuleIfCurrent(requireName) {
        let path = Util.getMainModulePath();
        let pckg = Util.getPackageJson(Path.join(path, 'package.json'));
        if (pckg) {
            if (requireName === pckg.name) {
                let mainPath = Path.join(path, pckg.main);
    ...
    create(settings) {
        settings = settings || {};
        const serialize = !!settings.serializations;
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            let requireName = this._componentDefinition.requireName.value;
            ...
            if (this._componentDefinition.requireElement) {
                ...
                if (serialize)
                    serialization += '.' + this._componentDefinition.requireElement.value;
                try {
                    subObject = requireElementPath.reduce((object, requireElement) => object[requireElement], object);
                }
                catch (e) {
            ...
            if (!this._componentDefinition.requireNoConstructor || this._componentDefinition.requireNoConstructor.value !== 'true') {
                if (this._constructable) {
                    ...
                    try {
                        const args = yield this.makeArguments(settings);
                        if (serialize) {
