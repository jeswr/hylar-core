...
exports.skolemize = exports.isBNode = exports._createFactSetFromTriples = exports.parseRule = exports.parseRules = exports.uniquesCausedBy = exports.unify = exports.unifyFactSet = exports.disjunctCauses = exports.combineImplicitCauses = exports.addConsequences = exports.buildCauses = exports.addAlternativeDerivationAsCausedByFromExplicit = exports.updateValidTags = exports.isVariable = exports.minus = exports.containsFacts = exports.similarAtoms = exports.causeMemberMatchesFactMember = exports.causeMatchesFact = exports.restrictRuleSet = exports.getOnlyExplicitFacts = exports.getOnlyImplicitFacts = exports.combine = void 0;
 ...
 * @returns {Array}
 */
function getOnlyImplicitFacts(fs) {
    const fR = [];
    for (const key in fs) {
        const fact = fs[key];
        if (!fact.explicit) {
...
exports.getOnlyImplicitFacts = getOnlyImplicitFacts;
/**
 * Returns explicit facts from the set.
 * @param fs
 * @returns {Array}
 */
function getOnlyExplicitFacts(fs) {
    const fR = [];
    for (const key in fs) {
        const fact = fs[key];
        if (fact.explicit) {
...
exports.getOnlyExplicitFacts = getOnlyExplicitFacts;
/**
 ...
 * @returns {Array}
 */
function restrictRuleSet(rs, fs) {
    const restriction = [];
...
exports.restrictRuleSet = restrictRuleSet;
/**
 ...
 * @returns {*}
 */
function causeMatchesFact(fact1, fact2) {
...
exports.causeMatchesFact = causeMatchesFact;
/**
...
exports.containsFacts = containsFacts;
/**
 * Substracts each set.
...
function updateValidTags(kb, additions, deletions) {
    const newAdditions = [];
...
exports.updateValidTags = updateValidTags;
function addAlternativeDerivationAsCausedByFromExplicit(kb, { consequences }, altFact) {
...
exports.addAlternativeDerivationAsCausedByFromExplicit = addAlternativeDerivationAsCausedByFromExplicit;
function buildCauses(conjunction) {
...
exports.buildCauses = buildCauses;
function addConsequences(facts, consequences) {
...
exports.addConsequences = addConsequences;
function combineImplicitCauses(implicitFacts) {
...
exports.combineImplicitCauses = combineImplicitCauses;
function disjunctCauses(prev, next) {
    let conjunction;
    const disjunction = [];
    if ((prev === []) || (next === [])) {
...
exports.disjunctCauses = disjunctCauses;
function unifyFactSet(fs) {
    const unifiedSet = [];
    let foundFactIndex;
...
exports.unifyFactSet = unifyFactSet;
function unify(subSet, updatingSet) {
...
exports.unify = unify;
function uniquesCausedBy(cb1, cb2) {
    let min;
    let max;
    const newCb = [];
    let found;
...
exports.uniquesCausedBy = uniquesCausedBy;
function parseRules(strRuleList, entailment = Rule_1.default.types.CUSTOM) {
...
function parseRule(strRule, name = `rule-${md5_1.default(strRule)}`, entailment) {
    const bodyTriples = strRule.split('->')[1].match(RegularExpressions_1.default.TRIPLE);
    const headTriples = strRule.split('->')[0].match(RegularExpressions_1.default.TRIPLE);
    const causes = _createFactSetFromTriples(headTriples);
    const consequences = _createFactSetFromTriples(bodyTriples);
    return new Rule_1.default(causes, consequences, name, entailment);
}
exports.parseRule = parseRule;
function _createFactSetFromTriples(triples) {
    const set = [];
    if (triples[0] === 'false') {
        // @ts-ignore
        set.push(new Fact_1.default('FALSE'));
    }
    else {
        for (let i = 0; i < triples.length; i++) {
            const atoms = [];
            for (let atom of triples[i].match(RegularExpressions_1.default.ATOM).splice(1)) {
                if (atom.match(RegularExpressions_1.default.PREFIXED_URI)) {
                    atom = Prefixes_1.default.replacePrefixWithUri(atom, atom.match(RegularExpressions_1.default.PREFIXED_URI)[1]);
                }
                atoms.push(atom);
            }
            set.push(new Fact_1.default(atoms[1], atoms[0], atoms[2]));
        }
    }
    return set;
}
exports._createFactSetFromTriples = _createFactSetFromTriples;
function isBNode(elem) {
    return ((elem !== undefined) && (elem.indexOf('__bnode__') === 0));
}
exports.isBNode = isBNode;
function skolemize(facts, elem) {
    let skolem = '';
    for (let i = 0; i < facts.length; i++) {
        skolem += facts[i].asString;
    }
    return md5_1.default(skolem) + elem;
}
